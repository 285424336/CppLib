#ifndef UDP_SCAN_TASK__H_INCLUDED
#define UDP_SCAN_TASK__H_INCLUDED
...
namespace UDPScan
{
    struct probespec_udpdata {
        unsigned short dport; //udp dst port
    };

    typedef struct probespec {
        struct probespec_udpdata udp; // udp dst info
    } probespec;

    struct IPExtraProbeData_udp {
        unsigned short sport; //udp src port
    };

    struct IPExtraProbeData {
        unsigned int ipid; // ip identify
        struct IPExtraProbeData_udp udp; // tcp src data
    };
...
#endif
#define UDP_SCAN_HOST_TIME_OUT_MS 3000
    ...
    {
    public:
        static bool InitPayloadsFromFile(const std::string &file);
        static bool InitPayloadsFromContent(const std::string &content);

    public:
        UDPScanTask(const std::vector<unsigned short> &ports, int src_ip, int dst_ip, char *src_mac, char *nxt_hop_mac, time_t time_out_ms = UDP_SCAN_HOST_TIME_OUT_MS, bool use_cwnd = false);
        ~UDPScanTask();
    ...
    private:
        ...
        *send tcp probe out
        */
        int SendUDPProbe( int ttl, bool df, unsigned short ipid, unsigned char* ipopt, int ipoptlen,
        ...
        */
        ...
        /**
        *check if the udp responce is match the probe
        */
        bool UDPProbeMatch(IPv4Header &ipv4_header, UDPHeader &udp_header, const UltraProbe &probe);
        /**
        *check if the icmp udp responce is match the probe
        */
        bool UDPProbeMatchUnreach(const IPv4Header &ipv4_header, const UDPHeader &udp_header, const UltraProbe &probe);
        /**
    ...
    private:
        static std::map<unsigned short, std::shared_ptr<std::string>> payloads; //udp pauloads used to send probe
    };
